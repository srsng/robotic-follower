...
class RobotInterface:
    ...
    def move_to_pose(
        self, position: list[float], orientation: list[float], wait: bool = True
    ) -> bool:
        """移动到目标位姿
        ...
        try:
            self.moveit_interface.move_to_pose(position=position, quat_xyzw=orientation)
            if wait:
    ...
    def move_to_joint_positions(
        self, positions: list[float], wait: bool = True
    ) -> bool:
        """移动到关节位置

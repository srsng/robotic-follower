...
class DetectionVisualizer:
    ...
    def visualize(
        ...
        if points.shape[1] >= 6:
            ...
            # 获取 RGB 图像数据 (优先使用 rgb_image_data)
            rgb_image = None
            if rgb_image_data is not None:
                rgb_image = rgb_image_data
                # 如果是 BGR 转换为 RGB
                if rgb_image.shape[-1] == 3:
                    rgb_image = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2RGB)
            elif rgb_image_path and os.path.exists(rgb_image_path):
                try:
                    img_tmp = cv2.imread(rgb_image_path)
                    if img_tmp is not None:
                        rgb_image = cv2.cvtColor(img_tmp, cv2.COLOR_BGR2RGB)
                except Exception as e:
                    print(f"⚠ 警告: 加载 RGB 图像失败: {e}")

            # 如果提取到了 RGB 图像，尝试应用颜色
            if rgb_image is not None:
                colors = self._map_image_to_pointcloud(
                    points, rgb_image, calib_path, depth2img
                )
                if colors is not None:
                    pcd.colors = o3d.utility.Vector3dVector(colors)
                    print("✓ 成功将 RGB 图像颜色映射到点云")
                else:
        ...
        if os.path.exists(font_path):
            try:
                ...
                if hasattr(app, "set_font"):
                    ...
                    print("当前 Open3D 版本不支持全局替换默认字体，将尝试使用 add_font")
            except Exception as e:
                print(f"设置中文字体失败: {e}")
        else:
            print(
                "未找到中文字体，如果出现乱码请安装字体，例如: sudo apt install fonts-noto-cjk"
            )
        ...
        scrollable_list = gui.ScrollableVert(
            0,
            gui.Margins(int(0.25 * em), int(0.25 * em), int(0.25 * em), int(0.25 * em)),
        )
        # 设置可滚动区域的高度
        ...
        def on_layout(layout_context):
            ...
            y_offset = (
                cb_labels.frame.height
                + cb_image.frame.height
                + image_widget.frame.height
                + 4 * em
            )
        ...
        try:
            if rgb_image is not None:
                # 把 numpy array 转换为 Open3D 图像
                # 注意 rgb_image 此时已经是 RGB 格式 (H, W, 3)
                self.rgb_image_o3d = o3d.geometry.Image(rgb_image)
                image_widget.update_image(self.rgb_image_o3d)
        except Exception as e:
            print(f"⚠ 警告: 无法将图像数据转换为用于 UI 显示的格式: {e}")

        def on_cb_labels_changed(is_checked):
            self._labels_enabled = is_checked
            widget3d.set_on_key(
                on_key_event
            )  # Dummy call, just trigger re-render? No, use setup
        ...
        def on_key_event(event):
            ...
            if event.type == gui.KeyEvent.DOWN:
                ...
                if event.key == gui.KeyName.LEFT:
                    ...
                    app.quit()
                    return True
                if event.key == gui.KeyName.ESCAPE or event.key == gui.KeyName.Q:
                    self.nav_action = 0
                    app.quit()
                    return True
            return False
    ...
    def _map_image_to_pointcloud(
        ...
        try:
            ...
            if depth2img is not None:
                ...
                u[valid_z_mask] = (pts_img[valid_z_mask, 0] / z[valid_z_mask]).astype(
                    int
                )
                v[valid_z_mask] = (pts_img[valid_z_mask, 1] / z[valid_z_mask]).astype(
                    int
                )
                ...
                if np.sum(final_mask) > 0:
                    ...
                    print(
                        f"✓ 使用 depth2img 矩阵成功映射点云颜色 (有效点: {np.sum(final_mask)})"
                    )
                    return colors
                ...
                if len(lines) >= 2:
                    ...
                    if len(Rt_vals) == 9 and len(K_vals) == 9:
                        ...
                        # 从图像提取颜色（归一化到 [0, 1]）
                        colors[final_mask] = (
                            rgb_image[v[final_mask], u[final_mask]] / 255.0
                        )
                        return colors

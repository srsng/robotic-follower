...
class CalibrationManager:
    ...
    def __init__(self, config: CalibrationConfig, node: Node | None = None):
        """初始化标定管理器。
    ...
    def add_sample(
        self, robot_pose: np.ndarray, image: np.ndarray, camera_pose: np.ndarray
    ) -> bool:
        """添加标定样本。
    ...
    def execute_calibration(self) -> dict:
        ...
        if len(self.samples) < self.config.min_samples:
            raise ValueError(
                f"样本数量不足：{len(self.samples)} < {self.config.min_samples}"
            )
    ...
    def save_results(self, filepath: str) -> bool:
        ...
        try:
            ...
            with open(filepath, "w") as f:
                yaml.dump(
                    result_to_save, f, default_flow_style=False, allow_unicode=True
                )
